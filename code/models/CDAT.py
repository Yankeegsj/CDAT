...
def get_optimizer(model,opt):
    ...
    if opt.optimizer.lower()=='adam':
        optimizer = torch.optim.Adam([{'params':main_layers, 'lr': opt.main_lr_init,'weight_decay': opt.main_weight_decay},
                                     {'params':secondary_layers, 'lr': opt.secondary_lr_init,'weight_decay': opt.secondary_weight_decay}])
    elif opt.optimizer.lower()=='sgd':
        optimizer = torch.optim.SGD([{'params':main_layers, 'lr': opt.main_lr_init,'weight_decay': opt.main_weight_decay},
                                     {'params':secondary_layers, 'lr': opt.secondary_lr_init,'weight_decay': opt.secondary_weight_decay}])
    return optimizer
...
class all_local_maximum_location_get(nn.Module):
    def __init__(self,k_size,thd):
        super(all_local_maximum_location_get, self).__init__()
        assert k_size%2==1
        self.max_pool=nn.MaxPool2d(kernel_size=k_size, stride=1,padding=int(k_size/2))
    ...
    def forward(self,x):
        ...
        pred_cnt=torch.sum(y_,(-2,-1)).item()
        return y_,pred_cnt
...
class get_den_level_label(nn.Module):
    def __init__(self, opt):
        super(get_den_level_label, self).__init__()
        self.opt=opt
        self.split_den_level=opt.split_den_level
        self.k1= nn.AvgPool2d(opt.grid, stride=opt.grid)

    def forward(self, den_map):
        batch=len(den_map)
        label_list=[]
        for i in range(batch):
            local_cnt=self.k1(den_map[i].detach().squeeze().unsqueeze(0).unsqueeze(0).cuda())*self.opt.grid[0]*self.opt.grid[1]/self.opt.gt_factor
            ...
            label=torch.stack(den_level_map,0)#level*h*w
            if len(label.size())==1:
...
class CDAT(nn.Module):
    def __init__(self, opt):
        ...
        teacher_params = []
        for key, value in dict(self.teacher_model.named_parameters()).items():
            if value.requires_grad:
                teacher_params += [value]
                value.requires_grad = False

        student_params = []
        for key, value in dict(self.student_model.named_parameters()).items():
            if value.requires_grad:
        ...
        if opt.model_for_load_MT:
            checkpoint=torch.load(opt.model_for_load_MT)
    ...
    def forward(self, data,search_time,epoch,global_step,step,vision_list):
...
class get_den_level_label_1(nn.Module):
    def __init__(self, grid,split,opt):
        super(get_den_level_label_1, self).__init__()
        self.opt=opt
        self.split_den_level=split
        self.grid=grid
        self.k1= nn.AvgPool2d(self.grid, stride=self.grid)

    def forward(self, den_map):
        batch=len(den_map)
        label_list=[]
        for i in range(batch):
            local_cnt=self.k1(den_map[i].detach().squeeze().unsqueeze(0).unsqueeze(0).cuda())*self.grid[0]*self.grid[1]/self.opt.gt_factor
            local_cnt=local_cnt.squeeze()
            den_level_map=[]
            for j in range(len(self.split_den_level)-1):
                tmp=((local_cnt>self.split_den_level[j])*(local_cnt<self.split_den_level[j+1])).float()
...
class VGG_16_deconv_DA_den_level_fix_BCE_BF_cls(nn.Module):
    def __init__(self, opt):
        ...
        self.mix_grid_avg_pool=nn.AvgPool2d(opt.mix_grid,opt.mix_grid)
        self.local_density_grid_avg_pool=nn.AvgPool2d(opt.local_density_grid,opt.local_density_grid)
        ...
        if self.opt.train_mode=='step':
            self.scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer, self.opt.decay_iter_freq, gamma=self.opt.decay_gamma)
    ...
    def get_mix_gt(self,src_den_map,src_gt_map):
        ...
        den_maps_gaussian=[]
        for i in range(batch):
            points_map,num_points=self.get_max_point(src_den_map[i:i+1].detach())
            points_map=points_map.float()
            points_gass_map=self.gen_map(points_map.unsqueeze(0).unsqueeze(0))*self.opt.gt_factor
            den_maps_gaussian.append(points_gass_map)

        den_maps_gaussian=torch.cat(den_maps_gaussian,0)
        src_local_den_map=self.mix_grid_avg_pool(src_den_map.detach())
        src_gt_local_den_map=self.mix_grid_avg_pool(torch.stack(src_gt_map,0).unsqueeze(1).cuda())
        src_local_gass_den_map=self.mix_grid_avg_pool(den_maps_gaussian)

        error1=torch.abs(src_local_den_map-src_gt_local_den_map)
        error2=torch.abs(src_local_gass_den_map-src_gt_local_den_map)

        mix_label = torch.zeros(size=src_gt_local_den_map.size()).cuda()
        mix_label[error1 <= error2] = 1.0

        src_local_den_map=self.avg_pool8(src_den_map.detach())*64
    ...
    def get_local_for_teacher_inference(self,den_map,gass_den_map):
        ...
        local_den_map=self.mix_grid_avg_pool(den_map.detach())
        out_shape=local_den_map.shape[-2:]

        local_den_map=self.avg_pool8(den_map.detach())*64
        local_gass_den_map=self.avg_pool8(gass_den_map)*64

        return local_den_map,local_gass_den_map,out_shape

    def forward(self, data,search_time,epoch,global_step,step,vision_list,tar_teacher_den_pred,tar_teacher_den_pred_gass,tar_teacher_den_mix,tar_BF_plabel,tar_ld_plabel,mae_src_teacher,mae_src_teacher_points,mae_src_teacher_mix):
        ...
        if batch>1:
            src_img=torch.stack(src_data[1])
            tar_img=torch.stack(tar_data[1])
        else:
            src_img=src_data[1][0].unsqueeze(0)
            tar_img=tar_data[1][0].unsqueeze(0)


        src_BF_gt=self.get_BF_label(src_data[2])
        src_BF_gt=torch.stack(src_BF_gt,0).detach()

        src_fea=self.backbone(src_img.cuda())
        src_den_map=self.den_pred(src_fea)
        src_BF=self.BF_cls(src_fea,src_BF_gt.shape[-2:])

        src_ld_gt=self.local_density_grid_avg_pool(torch.stack(src_data[2],0))*self.opt.local_density_grid[0]*self.opt.local_density_grid[1]
        ...
        if self.opt.concat_way=='only_density_map':
            inp=torch.cat([src_local_den_map,src_local_gass_den_map],dim=1)
        elif self.opt.concat_way=='only_feature_map':
            inp=src_fea
        elif self.opt.concat_way=='all':
            inp=torch.cat([src_fea,src_local_den_map,src_local_gass_den_map],dim=1)
        ...
        tar_BF_gt=torch.stack(tar_BF_gt,0).detach()

        tar_fea=self.backbone(tar_img.cuda())
        ...
        self.loss.update(loss.detach().item(),batch)
        self.D_src_loss.update(loss_D_src.detach().item(),batch)
        self.D_tar_loss.update(loss_D_tgt.detach().item(),batch)
        self.den_loss.update(den_loss.detach().item(),batch)
        self.p_label_loss.update(p_label_den_loss.detach().item(),batch)
        self.p_label_gass_loss.update(p_label_gass_den_loss.detach().item(),batch)
        self.p_label_mix_loss.update(p_label_mix_den_loss.detach().item(),batch)
        self.src_local_density_loss.update(src_local_density_loss.detach().item(),batch)
        self.src_BF_loss.update(src_BF_loss.detach().item(),batch)
        self.tar_local_density_loss.update(tar_local_density_loss.detach().item(),batch)
        self.tar_BF_loss.update(tar_BF_loss.detach().item(),batch)
        self.mix_loss.update(mix_loss.detach().item(),batch)
        self.mask_den_loss.update(mask_den_loss.detach().item(),batch)
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()

        if self.opt.BF_loss_choose.lower()=='bce':
            src_mask_pred=(F.sigmoid(src_BF.detach())>0.5).float()
            tar_mask_pred=(F.sigmoid(tar_BF.detach())>0.5).float()
        elif self.opt.BF_loss_choose.lower()=='softmax':
            src_BF_softmax=F.softmax(src_BF.detach(),dim=1)
            src_mask_pred=torch.argmax(src_BF_softmax,dim=1).unsqueeze(1).float()
            tar_BF_softmax=F.softmax(tar_BF.detach(),dim=1)
            tar_mask_pred=torch.argmax(tar_BF_softmax,dim=1).unsqueeze(1).float()

        self.BF_acc_src.update(torch.sum(src_BF_gt[src_BF_gt>0]==src_mask_pred[src_BF_gt>0]).item()/src_BF_gt[src_BF_gt>0].numel(),src_BF_gt[src_BF_gt>0].numel())
        self.BF_acc_tar.update(torch.sum(tar_BF_gt[tar_BF_gt>0]==tar_mask_pred[tar_BF_gt>0]).item()/tar_BF_gt[tar_BF_gt>0].numel(),tar_BF_gt[tar_BF_gt>0].numel())

        for i in range(batch):

            sum_map_src=(torch.sum(src_den_map[i].detach().cpu())/self.opt.gt_factor).item()
            self.mae_src.update(abs(sum_map_src-src_data[3][i]['gt_cnt']))
            self.mse_src.update((abs(sum_map_src-src_data[3][i]['gt_cnt'])**2))

            sum_map_tar=(torch.sum(tar_den_map[i].detach().cpu())/self.opt.gt_factor).item()
            ...
            if epoch<=self.opt.save_start_epochs or epoch%self.opt.vision_frequency==0:
                if step in vision_list and i==0:
                    ...
                    save_filename=os.path.join(save_root,video_name+'gt_point_',img_name.split('.')[0]+'_cnt_'+str(src_data[3][i]['gt_cnt'])+'.'+img_name.split('.')[1])
                    save_keypoints_img(src_img[i],src_data[2][i], self.opt, save_filename)
                    save_filename=os.path.join(save_root,video_name+'out_point_',img_name.split('.')[0]+'_cnt_'+str(sum_map_src)+'.'+img_name.split('.')[1])
                    save_keypoints_img(src_img[i],src_den_map[i].detach().squeeze().cpu(), self.opt, save_filename)
                    ...
                    save_filename=os.path.join(save_root,video_name+'gt_point_',img_name.split('.')[0]+'_cnt_'+str(tar_data[3][i]['gt_cnt'])+'.'+img_name.split('.')[1])
                    save_keypoints_img(tar_img[i],tar_data[2][i], self.opt, save_filename)
                    save_filename=os.path.join(save_root,video_name+'out_point_',img_name.split('.')[0]+'_cnt_'+str(sum_map_tar)+'.'+img_name.split('.')[1])
                    save_keypoints_img(tar_img[i],tar_den_map[i].detach().squeeze().cpu(), self.opt, save_filename)
                    ...
                    save_filename=os.path.join(save_root,video_name+'out_point_teacher',img_name.split('.')[0]+'_cnt_'+str(torch.sum(tar_teacher_den_pred[i])/self.opt.gt_factor)+'.'+img_name.split('.')[1])
                    save_keypoints_img(tar_img[i],tar_teacher_den_pred[i].squeeze().cpu(), self.opt, save_filename)

                    save_filename=os.path.join(save_root,video_name+'out_point_teacher',img_name.split('.')[0]+'_cnt_points_'+str(torch.sum(tar_teacher_den_pred_gass[i])/self.opt.gt_factor)+'.'+img_name.split('.')[1])
                    save_keypoints_img(tar_img[i],tar_teacher_den_pred_gass[i].squeeze().cpu(), self.opt, save_filename)

                    save_filename=os.path.join(save_root,video_name+'out_point_teacher',img_name.split('.')[0]+'_cnt_mix_'+str(torch.sum(tar_teacher_den_mix[i])/self.opt.gt_factor)+'.'+img_name.split('.')[1])
                    save_keypoints_img(tar_img[i],tar_teacher_den_mix[i].squeeze().cpu(), self.opt, save_filename)
    ...
    def teacher_inference(self, data,search_time,epoch,global_step,step,vision_list):
        ...
        with torch.no_grad():
            ...
            for i in range(batch):
                ...
                if self.opt.BF_loss_choose.lower()=='bce':
                    BF = (torch.sigmoid(BF.detach()) > 0.5).float()

                if self.opt.BF_loss_choose.lower()=='softmax':
                    BF = F.softmax(BF.detach(), dim=1)
                    BF = torch.argmax(BF, dim=1).unsqueeze(1).float()

                ld=self.Local_den_predictor(x,(int(x.size(-2)*8/self.opt.local_density_grid[0]),int(x.size(-1)*8/self.opt.local_density_grid[1])))

                den_map_detach=den_map.detach()
                sum_map=torch.sum(den_map.detach().cpu())/self.opt.gt_factor
                
                self.mae_src_teacher.update(abs(sum_map-data[3][i]['gt_cnt']))
                # den_maps.append(den_map.detach())

                points_map,num_points=self.get_max_point(den_map.detach())
                points_map=points_map.float()
                ...
                if self.opt.concat_way=='only_density_map':
                    inp=torch.cat([local_den_map,local_gass_map],dim=1)
                elif self.opt.concat_way=='only_feature_map':
                    inp=x
                elif self.opt.concat_way=='all':
                    inp=torch.cat([x,local_den_map,local_gass_map],dim=1)
                mix_pred=self.mix_cls(inp,out_shape)

                mix_map=torch.zeros(points_gass_map.shape).cuda()
                mix_pred=F.interpolate(mix_pred,size=mix_map.shape[-2:],mode='bilinear', align_corners=True)

                if self.opt.ada_mix_loss_choose.lower()=='bce' and self.opt.mix_mode=='soft':
                    mix_pred=torch.sigmoid(mix_pred.detach())
                    mix_map=mix_pred*den_map+(1-mix_pred)*points_gass_map
                if self.opt.ada_mix_loss_choose.lower()=='bce' and self.opt.mix_mode=='hard':
                    mix_pred = (torch.sigmoid(mix_pred.detach()) > 0.5).float()
                    mix_map=mix_pred*den_map+(1-mix_pred)*points_gass_map

                if self.opt.ada_mix_loss_choose.lower()=='softmax' and self.opt.mix_mode=='soft':
                    mix_pred = F.softmax(mix_pred.detach(), dim=1)
                ...
                if self.opt.ada_mix_loss_choose.lower()=='softmax' and self.opt.mix_mode=='hard':
                    mix_pred = F.softmax(mix_pred.detach(), dim=1)
                    mix_pred = torch.argmax(mix_pred, dim=1).unsqueeze(1).float()
                    mix_map=mix_pred*den_map+(1-mix_pred)*points_gass_map

                sum_mix_map=torch.sum(mix_map.cpu())/self.opt.gt_factor
                ...
                if 'HorizontallyFlip' in list(data[3][i].keys()):
                    if data[3][i]['HorizontallyFlip']:
                        den_map_detach = torch.flip(den_map_detach, [3])
                        mix_map = torch.flip(mix_map, [3])
                        points_gass_map = torch.flip(points_gass_map, [3])
                        BF = torch.flip(BF, [3])
                        ld = torch.flip(ld, [3])

                if 'VerticallyFlip' in list(data[3][i].keys()):
                    if data[3][i]['VerticallyFlip']:
                        den_map_detach = torch.flip(den_map_detach, [2])
                        mix_map = torch.flip(mix_map, [2])
                        points_gass_map = torch.flip(points_gass_map, [2])
                        BF = torch.flip(BF, [2])
                        ld = torch.flip(ld, [2])
    ...
    def inference(self,data,search_time,epoch,global_step,step,vision_list,batch_num_each_epoch,logger):
        ...
        for i in range(batch):
            ...
            if len(data)==5:
                scene=data[3][i]['img_file'][:6]
                mask=torch.from_numpy(data[4][i]).float()
                mask = F.interpolate(mask.unsqueeze(0).unsqueeze(0),size=(h,w),mode=self.opt.img_interpolate_mode,align_corners = True)
                mask = mask.squeeze()
                sum_map=torch.sum(den_map[i].detach().cpu()*mask)/self.opt.gt_factor
                if scene=='104207':
            ...
            if epoch<=self.opt.save_start_epochs or epoch%self.opt.vision_frequency==0:
                if step in vision_list and i==0:
                    ...
                    save_keypoints_img(data[0][i],den_map[0,0].detach().cpu(), self.opt, save_filename,img_alpha=0.0)

                    save_filename=os.path.join(save_root,video_name+'input_img_',img_name.split('.')[0]+'.'+img_name.split('.')[1])
...
class den_mse_loss(nn.Module):
    def __init__(self,opt):
        super(den_mse_loss, self).__init__()
        self.opt=opt
        # k_size=3
        self.mseloss=nn.MSELoss()
    def forward(self,src_pred_den_map,src_gt_map):
        batch=len(src_gt_map)
        if batch>1:
            src_gt_map=torch.stack(src_gt_map)
        else:
            src_gt_map=src_gt_map[0]

        den_loss=self.mseloss(src_pred_den_map.squeeze(),src_gt_map.cuda().squeeze())
...
class loss1(nn.Module):
    def __init__(self,opt):
        ...
        assert len(self.epoch_stage)==len(self.domain_loss_trade_off_list)+1

    def forward(self,epoch,src_den_map,tar_den_map,src_D_pred,tar_D_pred,src_gt_map,
        ...
        if self.den_loss_trade_off==0.:
            den_loss=torch.zeros(1).cuda()
        else:
            for i in range(batch):
                if i==0:
                    den_loss=self.mseloss(src_den_map[i].squeeze(),src_gt_map[i].cuda().squeeze())
                else:
                    den_loss+=self.mseloss(src_den_map[i].squeeze(),src_gt_map[i].cuda().squeeze())
            den_loss/=batch
        ...
        if self.src_BF_cls_weight==0.:
            src_BF_loss=torch.zeros(1).cuda()
        else:
            if self.opt.BF_loss_choose.lower()=='bce':
                src_BF_loss=F.binary_cross_entropy_with_logits(src_BF, src_BF_gt)
            elif self.opt.BF_loss_choose.lower()=='softmax':
                src_BF_loss=F.cross_entropy(src_BF, src_BF_gt.long().squeeze(1))
            src_BF_loss*=self.src_BF_cls_weight

        if self.tar_BF_cls_weight==0.:
            tar_BF_loss=torch.zeros(1).cuda()
        else:
            tar_BF_plabel=torch.cat(tar_BF_plabel,0)
            if self.opt.BF_loss_choose.lower()=='bce':
                tar_BF_loss=F.binary_cross_entropy_with_logits(tar_BF, tar_BF_plabel)
            elif self.opt.BF_loss_choose.lower()=='softmax':
                tar_BF_loss=F.cross_entropy(tar_BF, tar_BF_plabel.long().squeeze(1))
            tar_BF_loss*=self.tar_BF_cls_weight

        # domain loss
        if self.domain_loss_trade_off==0.:
            domain_loss=torch.zeros(1).cuda()
            src_loss=torch.zeros(1).cuda()
            tar_loss=torch.zeros(1).cuda()
        else:
            for i in range(batch):
                slabels = torch.ones(src_D_pred.size(), requires_grad=False).type(torch.float32).cuda()
                tlabels = torch.zeros(tar_D_pred[i].size(), requires_grad=False).type(torch.float32).cuda()
                if i==0:
                    tar_loss=F.binary_cross_entropy_with_logits(tar_D_pred[i],tlabels)
                    src_loss=F.binary_cross_entropy_with_logits(src_D_pred[i],slabels[i])
                else:
                    tar_loss+=F.binary_cross_entropy_with_logits(tar_D_pred[i],tlabels)
                    src_loss+=F.binary_cross_entropy_with_logits(src_D_pred[i],slabels[i])
            tar_loss/=batch
        ...
        if self.p_label_loss_trade_off==0:
            p_label_den_loss=torch.zeros(1).cuda()
        else:
            for i in range(batch):
                if i==0:
                    p_label_den_loss=self.mseloss(tar_den_map[i].squeeze(),den_plabel[i].cuda().squeeze())
                else:
                    p_label_den_loss+=self.mseloss(tar_den_map[i].squeeze(),den_plabel[i].cuda().squeeze())
        ...
        if self.p_label_gass_loss_trade_off==0:
            p_label_gass_den_loss=torch.zeros(1).cuda()
        else:
            for i in range(batch):
                if i==0:
                    p_label_gass_den_loss=self.mseloss(tar_den_map[i].squeeze(),gass_plabel[i].cuda().squeeze())
                else:
                    p_label_gass_den_loss+=self.mseloss(tar_den_map[i].squeeze(),gass_plabel[i].cuda().squeeze())
        ...
        if self.p_label_mix_loss_trade_off==0:
            p_label_mix_den_loss=torch.zeros(1).cuda()
        else:
            for i in range(batch):
                if i==0:
                    p_label_mix_den_loss=self.mseloss(tar_den_map[i].squeeze(),mix_plabel[i].cuda().squeeze())
                else:
                    p_label_mix_den_loss+=self.mseloss(tar_den_map[i].squeeze(),mix_plabel[i].cuda().squeeze())
        ...
        if self.src_local_density_loss_trade_off==0.:
            src_ldl=torch.zeros(1).cuda()
        else:
            src_ldl=self.mseloss(src_ld,src_ld_gt.cuda())
            src_ldl*=self.src_local_density_loss_trade_off

        if self.tar_local_density_loss_trade_off==0.:
            tar_ldl=torch.zeros(1).cuda()
        else:
            tar_ld_plabel=torch.cat(tar_ld_plabel,0)
            tar_ldl=self.mseloss(tar_ld,tar_ld_plabel)
            tar_ldl*=self.tar_local_density_loss_trade_off

        if self.mix_loss_trade_off==0.:
            mix_loss=torch.zeros(1).cuda()
        else:
            if self.opt.ada_mix_loss_choose.lower()=='bce':
                mix_loss=F.binary_cross_entropy_with_logits(mix_pred, mix_gt)
            elif self.opt.ada_mix_loss_choose.lower()=='softmax':
                mix_loss=F.cross_entropy(mix_pred, mix_gt.long().squeeze(1))
            mix_loss*=self.mix_loss_trade_off

        if self.mask_den_loss_trade_off==0.:
            mask_den_loss=torch.zeros(1).cuda()
        elif self.opt.mask_den_choose=='p':
            mask_plabel=self.get_BF_label(den_plabel)
            mask_plabel=torch.stack(mask_plabel,0).detach()
            mask_plabel = F.interpolate(mask_plabel,size=den_plabel[0].shape[-2:],mode='nearest')
            if self.opt.BF_loss_choose.lower()=='bce':
                tar_mask_pred=(F.sigmoid(tar_BF.detach())>0.5).float()
            elif self.opt.BF_loss_choose.lower()=='softmax':
                tar_BF_softmax=F.softmax(tar_BF.detach(),dim=1)
                tar_mask_pred=torch.argmax(tar_BF_softmax,dim=1).unsqueeze(1).float()
            tar_mask_pred = F.interpolate(tar_mask_pred,size=den_plabel[0].shape[-2:],mode='nearest')

            mask=torch.zeros(tar_mask_pred.size()).cuda()
            mask[tar_mask_pred==mask_plabel]=1.0

            mask_den_loss=self.mseloss(mask*tar_den_map,mask*torch.cat(den_plabel,0))
            mask_den_loss*=self.mask_den_loss_trade_off
        elif self.opt.mask_den_choose=='g':
            mask_plabel=self.get_BF_label(gass_plabel)
            mask_plabel=torch.stack(mask_plabel,0).detach()
            mask_plabel = F.interpolate(mask_plabel,size=gass_plabel[0].shape[-2:],mode='nearest')
            if self.opt.BF_loss_choose.lower()=='bce':
                tar_mask_pred=(F.sigmoid(tar_BF.detach())>0.5).float()
            elif self.opt.BF_loss_choose.lower()=='softmax':
                tar_BF_softmax=F.softmax(tar_BF.detach(),dim=1)
                tar_mask_pred=torch.argmax(tar_BF_softmax,dim=1).unsqueeze(1).float()
            tar_mask_pred = F.interpolate(tar_mask_pred,size=gass_plabel[0].shape[-2:],mode='nearest')

            mask=torch.zeros(tar_mask_pred.size()).cuda()
            mask[tar_mask_pred==mask_plabel]=1.0

            mask_den_loss=self.mseloss(mask*tar_den_map,mask*torch.cat(gass_plabel,0))
            mask_den_loss*=self.mask_den_loss_trade_off

        elif self.opt.mask_den_choose=='m':
            mask_plabel=self.get_BF_label(mix_plabel)
            mask_plabel=torch.stack(mask_plabel,0).detach()
            mask_plabel = F.interpolate(mask_plabel,size=mix_plabel[0].shape[-2:],mode='nearest')
            if self.opt.BF_loss_choose.lower()=='bce':
                tar_mask_pred=(F.sigmoid(tar_BF.detach())>0.5).float()
            elif self.opt.BF_loss_choose.lower()=='softmax':
                tar_BF_softmax=F.softmax(tar_BF.detach(),dim=1)
                tar_mask_pred=torch.argmax(tar_BF_softmax,dim=1).unsqueeze(1).float()
            tar_mask_pred = F.interpolate(tar_mask_pred,size=mix_plabel[0].shape[-2:],mode='nearest')

            mask=torch.zeros(tar_mask_pred.size()).cuda()
            mask[tar_mask_pred==mask_plabel]=1.0

            mask_den_loss=self.mseloss(mask*tar_den_map,mask*torch.cat(mix_plabel,0))
            mask_den_loss*=self.mask_den_loss_trade_off
